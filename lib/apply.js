...
export function applyPlan(plan, state) {
  ...
  if (planIsEmpty(plan) && !plan.needAgents && !plan.needPool) {
    return { installed, removed, depLines, errors };
  }
  ...
  if (plan.needAgents) {
    ...
    try {
      removeAgents(TOP_LEVEL_AGENTS, state.scope, projectRoot);
