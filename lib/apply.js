...
export function applyPlan(plan, state) {
  ...
  // Removals first, then installs.
  const removes = plan.skillOps.filter((o) => o.op === 'remove');
  const installs = plan.skillOps.filter((o) => o.op === 'install');

  removes.sort((a, b) => a.id.localeCompare(b.id));
  installs.sort((a, b) => a.id.localeCompare(b.id));

  for (const op of removes) {
    try {
      const ok = removeSkill(
        op.id,
        op.scope,
        op.scope === 'project' ? state.projectRoot : undefined,
      );
      if (ok) removed.push(op.id);
    } catch (e) {
      errors.push(`remove ${op.id}: ${e.message}`);
    }
  }
  ...
  for (const op of installs) {
    ...
    try {
      ...
      installed.push(op.id);
      installedIdsForDeps.push(op.id);
    } catch (e) {
      errors.push(`install ${op.id}: ${e.message}`);
    }
  }
  ...
  if (state.scope === 'global') {
    updateGlobalManifest({
      ...
      removedIds: [...new Set(removes.map((o) => o.id))],
      agents: agentsResult,
...
function updateGlobalManifest(delta) {
  if (delta.installedIds.length || delta.agents) {
    recordGlobalInstall({
