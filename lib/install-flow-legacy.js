...
export async function runLegacyInstallFlow(opts = {}) {
  ...
  p.note(
    [
      ...
      `Global target:  ~/.agents/`,
      `Project target: ${projectRoot}/.agents/`,
      `API keys always go to ~/.agents/settings.json`,
  ...
  const wantSearch = isCancel(
    await p.confirm({
      message: 'Install search skills globally? (ddg + brave + tavily → ~/.agents)',
      initialValue: true,
    }),
  );
    ...
    if (coreIds.length) {
      const coreScope = isCancel(
        await p.select({
          ...
          options: [
            { value: 'global', label: 'Globally', hint: '~/.agents (default)' },
            {
              value: 'project',
              label: 'This project only',
              hint: `${projectRoot}/.agents`,
            },
          ],
          initialValue: 'global',
        }),
      ...
      const choice = isCancel(
        await p.select({
          ...
          options: [
            ...
            { value: 'global', label: 'Install globally', hint: '~/.agents' },
            {
              value: 'project',
      ...
      if (skill.id === 'defectdojo-fix' && !hasDefectDojoConfig()) {
        if (!hasDefectDojoUrl()) {
          ...
          if (url && String(url).trim()) {
            ...
            p.log.success(
              'DEFECTDOJO_URL saved (not printed).',
            );
          } else {
            p.log.message(
              pc.dim(
