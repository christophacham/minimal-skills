...
export async function runClackWizard(opts = {}) {
  ...
  // Fresh project: seed CORE+SEARCH as selected (not applied).
  if (state.selected.size === 0) {
    for (const id of defaultSelectedSkillIds()) state.selected.add(id);
  }

  const existingIds = uniqueProjectSkillIds(installed);
  if (existingIds.length > 0) {
    const choice = isCancel(
      await p.select({
        message: `Found ${existingIds.length} project skill(s). Update from this package?`,
        options: [
          {
            value: 'update-continue',
            label: 'Update and continue',
            hint: 'overwrite project only · open wizard',
          },
          {
            value: 'update-exit',
            label: 'Update and exit',
            hint: 'overwrite project only · quit',
          },
          {
            value: 'skip',
            label: 'Continue without updating',
            hint: 'leave disk as-is',
          },
        ],
      }),
    );
    if (choice === 'update-continue' || choice === 'update-exit') {
      const result = refreshProjectSkills(projectRoot, {
        skipDeps: state.skipDeps,
      });
      p.log.success(
        `Updated ${result.refreshed.length} project placement(s) to package v${result.version}` +
          (result.errors.length ? ` · ${result.errors.length} error(s)` : ''),
      );
      for (const line of result.depLines) p.log.message(pc.dim(line));
      for (const err of result.errors) p.log.error(err);
      if (choice === 'update-exit') {
        p.outro(pc.green('Project skills refreshed.'));
        return;
      }
  ...
  p.outro(pc.dim('Done. Re-run: ') + pc.cyan(`npx -y ${installPin()}`));
}

function shortPath(s) {
  const home = userAgentsDir().replace(/\/\.agents$/, '');
...
function statusBlock(state, installed, plan) {
  ...
  const lines = [
    ...
    `target:   ${shortPath(destLabel(state.scope, state.projectRoot))}`,
...
function showStatus(state, installed) {
  ...
  for (const g of SKILL_GROUPS) {
    ...
    for (const s of g.skills) {
      ...
      const disk = skillStatus(installed, s.id, state.scope);
      const diskMark =
        disk === 'installed'
          ? pc.green('on')
          : pc.dim('off');
  ...
  if (skills.length > 6 || groupId === 'all') {
    const q = isCancel(
      await p.text({
        ...
        placeholder: 'e.g. design, search, architecture',
      }),
    );
...
async function changeScope(state, installed, projectRoot) {
      ...
      options: [
        {
          value: 'project',
          label: 'Project',
          hint: shortPath(`${projectRoot}/.agents/skills`),
        },
        {
          value: 'global',
          label: 'Global',
          hint: shortPath(`${userAgentsDir()}/skills`),
        },
      ],
  ...
  if (resync && state.selected.size === 0) {
    const seed = isCancel(
      await p.confirm({
        message:
          'Nothing installed there — seed default CORE+SEARCH selection?',
        initialValue: true,
      }),
    );
    if (seed) {
...
async function doApply(state, installed) {
  ...
  if (planIsEmpty(plan)) {
    ...
    // Still collect DefectDojo creds if the skill is in the cart and incomplete
    if (state.selected.has('defectdojo-fix') && !hasDefectDojoConfig()) {
      await maybePromptDefectDojo();
    }
    return;
  }
  ...
  // Collect DefectDojo creds before write when skill is selected and incomplete
  if (state.selected.has('defectdojo-fix') && !hasDefectDojoConfig()) {
    await maybePromptDefectDojo();
  }
  ...
  // Search keys only when those skills newly installed
  const installedIds = new Set(result.installed);
  if (installedIds.has('brave-search') && !hasBraveKey()) {
    await maybePromptBrave();
  }
  if (installedIds.has('tavily-search') && !hasTavilyKey()) {
...
async function maybePromptBrave() {
  ...
  if (key && String(key).trim()) {
    ...
    p.log.success('BRAVE_API_KEY saved (not printed).');
  }
}
...
async function maybePromptTavily() {
  ...
  if (key && String(key).trim()) {
    ...
    p.log.success('TAVILY_API_KEY saved (not printed).');
  }
}

/** Prompt URL then token when defectdojo-fix is active and either is missing. */
async function maybePromptDefectDojo() {
  if (hasDefectDojoConfig()) {
  ...
  if (!hasDefectDojoUrl()) {
    const url = isCancel(
      await p.text({
        message: `DEFECTDOJO_URL for defectdojo-fix (Enter skip) — e.g. http://host:8080`,
        placeholder: 'http://192.168.x.x:8080',
      }),
    );
    if (url && String(url).trim()) {
      setEnvKey('DEFECTDOJO_URL', String(url).trim());
      p.log.success('DEFECTDOJO_URL saved (not printed).');
    } else {
      p.log.message(
        pc.dim('DefectDojo URL skipped — skill may STATUS: BLOCKED until set.'),
      );
    }
  }
  if (!hasDefectDojoToken()) {
    const tok = isCancel(
      await p.password({
        message: `DEFECTDOJO_API_TOKEN for defectdojo-fix (Enter skip) — ${settingsPathForDisplay()}`,
      }),
    );
    if (tok && String(tok).trim()) {
      setEnvKey('DEFECTDOJO_API_TOKEN', String(tok).trim());
      p.log.success(
        'DEFECTDOJO_API_TOKEN saved (not printed).',
      );
    } else {
      p.log.message(
        pc.dim('DefectDojo token skipped — skill may STATUS: BLOCKED until set.'),
      );
    }
  }
}
...
async function manageKeys() {
  const dd = defectDojoConfigStatus();
  const ddLine =
    dd === 'ok'
      ? 'url + token set'
      : dd === 'partial'
  ...
  if (action === 'dd-url') await maybePromptDdUrlForce();
  if (action === 'dd-token') await maybePromptDdTokenForce();
}
...
async function maybePromptDdUrlForce() {
  const url = isCancel(
    await p.text({
      message: `DEFECTDOJO_URL (Enter cancel) — e.g. http://host:8080`,
      placeholder: 'http://192.168.x.x:8080',
    }),
  );
  if (url && String(url).trim()) {
    ...
    p.log.message(pc.dim('Left DefectDojo URL unchanged.'));
  }
}

async function maybePromptDdTokenForce() {
  const tok = isCancel(
    await p.password({
      message: `DEFECTDOJO_API_TOKEN (Enter cancel) — ${settingsPathForDisplay()}`,
    }),
  );
  if (tok && String(tok).trim()) {
    setEnvKey('DEFECTDOJO_API_TOKEN', String(tok).trim());
    ...
    p.log.message(pc.dim('Left DefectDojo token unchanged.'));
  }
}
