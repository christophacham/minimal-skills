...
export function createDesiredState(opts) {
  ...
  let trees;
  if (opts.trees) {
    trees = normalizeTrees(opts.trees);
  } else if (opts.seedFromInstalled) {
    // Match disk targets so startup is in sync (e.g. agents mirror present).
    trees = treesFromInstalled(opts.seedFromInstalled, scope);
  } else {
    trees = normalizeTrees(['claude']);
  }
  /** @type {Set<string>} */
  let selected;
  if (opts.selected) {
...
/**
 * Active skill trees that match what is on disk for a scope.
 * Fresh scope (nothing installed) → claude only. Agents tree included when any
 * agents placement exists so the cart does not invent −remove on startup.
 *
 * @param {InstalledSkill[]} installed
 * @param {Scope} scope
 * @returns {SkillTree[]}
 */
export function treesFromInstalled(installed, scope) {
  const hasAgents = installed.some(
    (x) => x.scope === scope && x.tree === 'agents',
  );
  return normalizeTrees(hasAgents ? ['claude', 'agents'] : ['claude']);
}

/**
 * Align trees + selected with disk for the state's current scope.
 * @param {DesiredState} state
 * @param {InstalledSkill[]} installed
 */
export function resyncFromInstalled(state, installed) {
  state.trees = treesFromInstalled(installed, state.scope);
  state.selected = new Set(
    installed
      .filter((x) => x.scope === state.scope && state.trees.includes(x.tree))
      .map((x) => x.id),
  );
  return state;
}
