 ...
 * Project installs are never manifest-tracked and are not touched here.
 */
export async function runUninstallFlow(opts = {}) {
  ...
  if (!manifestHasAnything(m)) {
    p.log.warn(
      `No tracked global installs in ${globalManifestPath()}\n` +
        ...
        'Use the wizard (deselect + Apply) to remove project installs.',
    );
  ...
  if (!opts.yes) {
    const ok = isCancel(
      await p.confirm({
        message: 'Remove only these tracked items from ~/.agents?',
        initialValue: true,
      }),
    );
    if (!ok) {
  ...
  clearManifest();
  spin.stop(`Removed ${count} tracked item(s) from ~/.agents`);

  p.note(
    'API keys in settings.json were left alone.\n' +
      'Project .agents/ installs were not touched.\n' +
