    ...
    const targetMeta = {
      ...
      'dd-url': {
        ...
        message: pendingApplyAfterKeys
          ? 'DEFECTDOJO_URL needed for defectdojo-fix (Enter save · empty skip · ESC cancel apply)'
      ...
      'dd-token': {
        ...
        mask: true,
        message: pendingApplyAfterKeys
          ? 'DEFECTDOJO_API_TOKEN needed for defectdojo-fix (Enter save · empty skip · ESC cancel apply) — never printed'
          : 'DEFECTDOJO_API_TOKEN (Enter save · ESC cancel) — never printed',
        step: 'token',
      },
    };
    const meta = targetMeta[keyTarget] || targetMeta.brave;
    body = h(TextPrompt, {
      message: meta.message,
      mask: meta.mask,
      onCancel: () => {
        if (pendingApplyAfterKeys) {
          setPendingApplyAfterKeys(false);
          setFlash('Apply cancelled — credentials not saved');
          setScreen('apply');
          refresh();
          return;
        }
      ...
      onSubmit: (v) => {
        if (v && v.trim()) {
          ...
          setFlash(`${meta.name} skipped — skill may STATUS: BLOCKED until set`);
        }
