...
export function App({ projectPath, skipDeps = false }) {
  ...
  const header = h(
    Box,
    {
      ...
      borderStyle: 'round',
      borderColor: counts.install + counts.remove ? 'yellow' : 'cyan',
      paddingX: 1,
      marginBottom: 1,
    },
    h(
      ...
      `claude-skills wizard  ·  ${state.scope}` +
    ...
    ...summarizePlan(plan).map((line, i) =>
      h(
        ...
        {
          key: `p-${i}`,
          color: line.startsWith('(no') ? undefined : 'yellow',
  ...
  if (screen === 'main') {
    body = h(List, {
      items: [
        ...
        },
        {
          label: 'Status detail',
          value: 'status',
          sublabel: counts.install + counts.remove ? `+${counts.install}/−${counts.remove}` : 'in sync',
          description: 'Selected (●) vs on-disk for every skill',
        },
        {
          label:
            counts.install + counts.remove
              ...
              : 'Apply changes',
          value: 'apply',
          sublabel: counts.install + counts.remove ? 'writes disk' : 'nothing pending',
          description: 'Review paths and apply the plan for this scope/targets',
        },
        {
          label: 'API keys',
          value: 'keys',
      ...
      onSelect: (value) => {
        if (value === 'browse') setScreen('browse-groups');
          ...
          if (counts.install + counts.remove > 0) setScreen('confirm-exit');
          else exit();
        }
      },
    });
    ...
    if (!skills.length) {
      ...
      body = h(MultiCheck, {
        ...
        items: skills.map((s) => {
          ...
          const mark =
            disk === 'installed' ? 'on disk' : disk === 'partial' ? 'partial' : 'off';
          return {
            id: s.id,
            label: s.label,
            hint: `${mark} · ${s.hint}`,
      ...
      for (const s of g.skills) {
        ...
        rows.push(
          h(
            Text,
            { key: s.id },
            `  ${want} ${s.id.padEnd(26)} ${diskMark.padEnd(8)} ${s.hint}`,
          ),
        );
      }
    }
    ...
    if (planIsEmpty(plan)) {
      ...
      body = h(
        ...
        h(Box, { height: 1 }, h(Text, null, ' ')),
        ...summarizePlan(plan).map((l, i) => h(Text, { key: `s-${i}`, color: 'yellow' }, l)),
        h(Box, { height: 1 }, h(Text, null, ' ')),
        ...
        h(List, {
          items: [
            {
              label: `Apply to ${state.scope}`,
              value: 'yes',
              description: 'Write skills / agents / pool / manifest as planned',
            },
