...
export function App({ projectPath, skipDeps = false }) {
  ...
  if (screen === 'main') {
    ...
    body = h(TextPrompt, {
      ...
      onCancel: () => {
        // Keep prior filter (if any) and return to the list
        setAwaitingFilter(false);
      },
      onSubmit: (q) => {
    ...
    body = h(MultiCheck, {
      ...
      // Seed from full cart so filtered confirm does not wipe hidden selections
      initialSelected: [...state.selected],
      onRequestFilter: () => setAwaitingFilter(true),
      onCancel: () => {
        setFilterQuery('');
        setAwaitingFilter(false);
      ...
      onConfirm: (ids) => {
        // Only update selection for skills currently in view; leave others alone.
