...
/**
 * @typedef {'update-existing'|'main'|'browse-groups'|'browse-skills'|'scope'|'status'|'apply'|'manage'|'keys'|'key-input'|'confirm-exit'|'global-uninstall'|'message'} Screen
 */
...
export function App({ projectPath, skipDeps = false }) {
  ...
  const [screen, setScreen] = useState(
    /** @type {Screen} */ (
      boot.ok && boot.existingIds.length > 0 ? 'update-existing' : 'main'
    ),
  );
  ...
  /** After project refresh message, Exit instead of main menu. */
  ...
  /**
   * Overwrite project suite skills from this package, then continue or exit.
   * @param {'continue'|'exit'} after
   */
  const runProjectRefresh = (after) => {
    setApplying(true);
    setTimeout(() => {
      try {
        ...
        const parts = [`${result.refreshed.length} updated`];
        if (result.errors.length) parts.push(`${result.errors.length} error(s)`);
        setFlash(`Project skills → v${result.version} · ${parts.join(' · ')}`);
        setMessage({
          title: `Updated project skills to v${result.version}`,
          lines: [
            ...result.refreshed.map((x) => `refresh ${x}`),
            ...result.depLines,
            ...result.errors.map((e) => `error: ${e}`),
            after === 'exit' ? 'Press Continue to exit.' : '',
          ].filter(Boolean),
        });
        setExitAfterMessage(after === 'exit');
        setScreen('message');
      } catch (e) {
        setFlash(`Update failed: ${e.message}`);
        setScreen('main');
      } finally {
        setApplying(false);
        refresh();
  ...
  if (screen === 'update-existing') {
    const ids = boot.existingIds;
    const preview =
      ids.length <= 8
        ? ids.join(', ')
    ...
    body = h(
      Box,
      { flexDirection: 'column' },
      h(Text, { bold: true, color: 'yellow' }, 'Existing project skills found'),
      h(
        Text,
        { dimColor: true },
        `This package is v${suiteVersion()}. Overwrite project copies only (never global).`,
      ),
      h(Text, { dimColor: true }, shortPath(projectRoot)),
      h(Text, null, `${ids.length} skill(s): ${preview}`),
      h(Box, { height: 1 }, h(Text, null, ' ')),
      applying
        ? h(Text, { color: 'yellow' }, 'Updating…')
        : h(List, {
            items: [
              {
                ...
                description:
                  'Overwrite project skills with this package, then open the wizard',
              },
              {
                label: 'Update and exit',
                value: 'update-exit',
                description:
                  'Overwrite project skills with this package, then quit',
              },
              {
                label: 'Continue without updating',
                value: 'skip',
                description: 'Leave existing project skills as-is; open the wizard',
              },
            ],
            onSelect: (value) => {
              if (value === 'skip') {
              ...
              runProjectRefresh(value === 'update-exit' ? 'exit' : 'continue');
            },
          }),
    );
  } else if (screen === 'main') {
      ...
      items: [
        {
          ...
          value: 'scope',
          sublabel: state.scope === 'project' ? shortPath(projectRoot) : '~/.agents',
          description: 'Where to install — project (default) or global ~/.agents/skills',
        },
        {
        ...
        '-',
        {
          ...
          description:
            'Brave / Tavily / DefectDojo → ~/.agents/settings.json only',
        },
        {
        ...
        items: [
          {
            label: 'Project',
            value: 'project',
            sublabel: shortPath(`${projectRoot}/.agents/skills`),
            description: 'Default — skills live with the repo',
          },
          {
            label: 'Global',
            value: 'global',
            sublabel: '~/.agents/skills',
            description: 'User-wide skills',
          },
        ],
        onSelect: (value) => {
              ...
              ? [
                  {
                    ...
                    value: 'dd-creds',
                    description: 'URL then token → ~/.agents/settings.json',
                  },
                ]
              : []),
    ...
    if (!manifestHasAnything(m)) {
      ...
      body = h(
        ...
        h(List, {
          items: [
            { label: 'Yes, remove from ~/.agents', value: 'yes' },
            { label: 'Cancel', value: 'no' },
          ],
          onSelect: (value) => {
            if (value === 'yes') {
    ...
    body = h(
      ...
      h(List, {
        items: [
          {
            label: exitAfterMessage ? 'Exit' : 'Continue',
            value: 'ok',
          },
        ],
        onSelect: () => {
          if (exitAfterMessage) exit();
          else goMain();
        },
      }),
    );
