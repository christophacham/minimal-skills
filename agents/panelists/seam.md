---
name: seam
description: Read-only design panelist. Argues for a behavior-preserving seam (an indirection, interface, or boundary) that makes the requested change live in one place without rippling. Use in a 3-panelist design round (with deep-module and minimal-diff) under a parent judge. Do not implement, do not edit files.
